...
class WC_Subscriptions_Cart {
	 ...
	 * - 'initial_total' used to calculate the initial total.
	 * - 'sign_up_fee_total' used to calculate the initial amount when there is a free trial period *and* a sign-up fee. Different to 'initial_total' because shipping is not charged on a sign-up fee.
	 * - 'recurring_total' used to calculate the totals for the recurring amount when the recurring amount differs to to 'initial_total' because of coupons or sign-up fees.
	 *
	 * @since 1.2
	 */
	...
	public static function set_subscription_prices_for_calculation( $price, $product ) {
		...
		if ( WC_Subscriptions_Product::is_subscription( $product ) ) {
			...
			if ( 'initial_total' == self::$calculation_type ) {
	...
	public static function calculate_subscription_totals( $total ) {
		...
		if ( $cart_sign_up_fee > 0 && ! $cart_has_trial ) {
			self::$calculation_type = 'initial_total';
			$woocommerce->cart->calculate_totals();
			$initial_cart = clone $woocommerce->cart;
		}

		// Now lets choose the cart with the appropriate total
		...
		// if there is a sign-up fee and NO free trial, the cart used to calculate initial totals holds the correct values
		if ( $cart_sign_up_fee > 0 && ! $cart_has_trial )
	...
	public static function set_calculated_total( $total ) {
		...
		$base_sign_up_fee = ( self::cart_contains_subscription_renewal() ) ? 0 : self::get_cart_subscription_sign_up_fee();

		if ( 'initial_total' == self::$calculation_type || 'sign_up_fee_total' == self::$calculation_type ) {
