...
class WC_Subscriptions_Order {
	...
	public static function calculate_next_payment_date( $order, $product_id, $type = 'mysql', $from_date = '' ) {
		...
		} elseif ( $subscription_trial_length > 0 && $trial_end_time > gmdate( 'U' ) + 120 ) {
	...
	public static function get_item_by_id( $order_item_id ) {
		...
		public static function get_prout_by_order( $id ) {
		global $wpdb;
	...
	$order = new WC_Order( $order_id );
		...
		$items['name']      = $items->order_item_name;
		$items['type']      = $items->order_item_type;
		$items['item_meta'] = $order->get_item_meta( $item->order_item_id );

		// Put meta into item array
		foreach( $items['item_meta'] as $meta_name => $meta_value ) {
			$key = substr( $meta_name, 0, 1 ) == '_' ? substr( $meta_name, 1 ) : $meta_name;
			$items[$key] = $meta_value[0];
		}
