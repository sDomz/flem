	...
	public function column_default( $item, $column_name ){
		...
		switch( $column_name ){
			...
			case 'title' :
				...
				$order      = new WC_Order( $item['order_id'] );
				foreach($order->get_items() as $test){
				//echo $item['order_id'];
				$column_content  .= sprintf( ' <a href="%s">%s</a> - ', get_edit_post_link( $test['product_id'] ), WC_Subscriptions_Order::get_item_name( $item['order_id'], $test['product_id'] ) );
	...
	function prepare_items() {
		...
		foreach ( $subscriptions_grouped_by_user as $user_id => $users_subscriptions ) {
			...
			foreach ( $users_subscriptions as $subscription_key => $subscription ) {
				if(false === array_search($subscription['order_id'], $comp))
				{
